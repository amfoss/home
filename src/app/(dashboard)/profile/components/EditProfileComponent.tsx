...
type EditProfileProps = {
  onCancel: () => void;
  initialData?: MemberProfileDetails | null;
  initialEnrollingState?: boolean;
}

export default function EditProfileComponent({ onCancel, initialData, initialEnrollingState }: EditProfileProps) {
  ...
  const [GenToggle, setGenToggle] = useState([
    initialData?.sex === "M" || false,
    initialData?.sex === "F" || false
  ]);
  const [previewUrl, setPreviewUrl] = useState<string>("/placeholder.webp");
  const [isUserEnrolling, setUserEnrolling] = useState(initialEnrollingState ?? false);
  ...
  useEffect(() => {
    ...
    if (!initialData) {
      async function getProfileDetails() {
        try {
          ...
          if (member) {
            ...
            if (!member.rollNo) {
              // if rollNo is not set, user hasn't updated their profile yet
              setUserEnrolling(true);
            } else {
              setGenToggle([member?.sex == "M", member?.sex == "F"]);
            }
          } else {
            // No profile exists - user is enrolling for the first time
            setUserEnrolling(true);
      ...
      // Initial data was provided, load profile image
      async function loadProfileImage() {
        if (initialData) {
          const url = await GetProfileService.HandleProfileImage(initialData);
          if (url != "") setPreviewUrl(url);
        }
      }
      loadProfileImage();
